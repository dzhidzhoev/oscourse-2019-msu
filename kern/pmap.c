...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		pages[i].pp_ref = 0;
		if ((i >= 1 && i < npages_basemem) || (uint32_t)nextfree <= (uint32_t)page2kva(pages + i)) {
			pages[i].pp_link = page_free_list;
			page_free_list = &pages[i];
		}
		// cprintf("%d %d %p %x\n", (i >= 1 && i < npages_basemem), (uint32_t)nextfree <= page2pa(pages + i), nextfree, page2pa(pages + i));
	}
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref != 0 || pp->pp_link != NULL) {
		panic("trying to free page with references on it or already freed");
	}
...
{
	if (va >= (void*)ULIM || (uintptr_t)va + len > ULIM) {
		user_mem_check_addr = MAX((uintptr_t)va, ULIM);
		return -E_FAULT;
	}

	uintptr_t va_low = (uintptr_t)ROUNDDOWN(va, PGSIZE);
	uintptr_t va_high = (uintptr_t)ROUNDUP(va + len, PGSIZE);
	for (uintptr_t va_cur = (uintptr_t)va_low; va_cur < va_high; va_cur += PGSIZE) {
		pte_t *pte;
		if (!(pte = pgdir_walk(env->env_pgdir, (void*)va_cur, 0))) {
			user_mem_check_addr = MAX((uintptr_t)va, va_cur);
			return -E_FAULT;
		}
