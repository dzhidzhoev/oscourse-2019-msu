...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	// address!
	user_mem_assert(e, tf, sizeof(*tf), PTE_P | PTE_U | PTE_W);
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	int r;
	if ((r = envid2env(srcenvid, &src_e, 1))) {
		return r;
	}
	if ((r = envid2env(dstenvid, &dst_e, 1))) {
		return r;
	}
	...
	pde_t *src_page;
	struct PageInfo *pp;
	if (!(pp = page_lookup(src_e->env_pgdir, srcva, &src_page))) {
		return -E_INVAL;
	}
	if ((perm & PTE_W) && !(*src_page & PTE_W)) {
		return -E_INVAL;
	}
	if (page_insert(dst_e->env_pgdir, pp, dstva, perm)) {
		return -E_NO_MEM;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((uintptr_t)srcva < UTOP) {
		...
		struct PageInfo *pp;
		pte_t *p;
		if (!(pp = page_lookup(curenv->env_pgdir, srcva, &p))) {
			return -E_INVAL;
		}
